...
export const verifyEmail = errorWrapper(async (req, res, next) => {
    ...
    if (user.verification[0].token.data !== emailVerificationString) return res.redirect(`${process.env.STUDENT_URL}email-verification/?success=false&info=invalidToken`);
