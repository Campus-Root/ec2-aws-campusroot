...
export const singleStudentProfile = errorWrapper(async (req, res, next) => {
    ...
    await courseModel.populate(student, [{ path: "recommendations.data.course activity.shortListed.course activity.wishList activity.products.course", select: "name discipline subDiscipline schoolName studyLevel duration applicationDetails university", },])
    ...
    await universityModel.populate(student, [{ path: "recommendations.data.university activity.shortListed.university activity.wishList.university activity.products.university", select: "name logoSrc location type establishedYear ", },])
