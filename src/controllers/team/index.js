...
export const singleStudentProfile = errorWrapper(async (req, res, next) => {
    ...
    await productModel.populate(student, [{ path: "activity.products orders.products" }]),    await courseModel.populate(student, [{ path: "recommendations.data.course activity.cart.course activity.wishList activity.products.course", select: "name discipline subDiscipline schoolName studyLevel duration applicationDetails university elite", },])
