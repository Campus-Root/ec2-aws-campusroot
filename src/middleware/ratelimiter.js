...
export const limiter = rateLimit({
  ...
  export const customRateLimiter = rateLimit({
    ...
    max: (req, res) => {
        ...
        return 3; // Maximum 3 requests allowed
    },
    handler: (req, res, next) => {
        const timeRemaining = req.rateLimit.nextAllowed - Date.now();
        if (timeRemaining > 0) return res.status(429).send({ message: `Too many requests from this IP, please try again after ${Math.ceil(timeRemaining / 1000)} seconds` });
        next();
    },
});